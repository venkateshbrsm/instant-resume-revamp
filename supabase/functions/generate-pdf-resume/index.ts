...
async function generateFallbackPdf(resumeData: any, templateId: string = 'modern', themeId: string = 'navy'): Promise<ArrayBuffer> {
  console.log('Generating fallback PDF with jsPDF');
  ...
  const nameText = resumeData.name || 'Professional Name';
  doc.text(nameText, margin, 25);
  
  doc.setFontSize(14);
  doc.setFont("helvetica", "normal");
  const titleText = resumeData.title || 'Professional Title';
  ...
  let contactY = 45;
  if (resumeData.email) {
    doc.text(`Email: ${resumeData.email}`, margin, contactY);
    contactY += 5;
  }
  if (resumeData.phone) {
    doc.text(`Phone: ${resumeData.phone}`, margin, contactY);
    contactY += 5;
  }
  if (resumeData.location) {
    doc.text(`Location: ${resumeData.location}`, pageWidth/2, 45);
  }
  if (resumeData.linkedin) {
    doc.text(`LinkedIn: ${resumeData.linkedin}`, pageWidth/2, 50);
  }
  ...
  if (resumeData.summary) {
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(0, 0, 0);
    currentY = addWrappedText(resumeData.summary, margin, currentY, contentWidth);
    currentY += 10;
  }
  ...
  if (resumeData.experience && resumeData.experience.length > 0) {
    ...
    resumeData.experience.forEach((exp: any) => {
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(0, 0, 0);
      const jobTitle = exp.title || exp.position || 'Position Title';
      ...
      const company = exp.company || 'Company Name';
      const duration = exp.duration || exp.dates || 'Duration';
      doc.text(`${company} | ${duration}`, margin, currentY);
      currentY += 8;
      
      doc.setTextColor(0, 0, 0);
      const description = exp.description || exp.responsibilities || 'Job responsibilities and achievements';
      currentY = addWrappedText(description, margin, currentY, contentWidth);
      currentY += 8;
    });
  }
  ...
  if (resumeData.education && resumeData.education.length > 0) {
    ...
    resumeData.education.forEach((edu: any) => {
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(0, 0, 0);
      const degree = edu.degree || edu.qualification || 'Degree/Qualification';
      ...
      doc.setFont("helvetica", "normal");
      doc.setTextColor(100, 100, 100);
      const institution = edu.institution || edu.school || 'Institution Name';
      const year = edu.year || edu.duration || 'Year';
      doc.text(`${institution} | ${year}`, margin, currentY);
      currentY += 8;
  ...
  if (resumeData.skills && resumeData.skills.length > 0) {
    ...
    currentY = addWrappedText(skillsText, margin, currentY, contentWidth);
  }
